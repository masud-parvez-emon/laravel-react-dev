...
import { Button, Skeleton, TableHead, Tooltip } from '@mui/material';
...
export default function Index() {
    ...
    const { data, isFetching } = useGetContainersQuery({
                                                        _page: page+1,
    ...
    return (
        <>
            <div className="my-2 flex justify-end">
                <Button variant="outlined" size='small' to="/containers/create" component={Link}>
                    Add Container
                </Button>
            </div>
            <Box>
                ...
                <TableContainer component={Paper}>
                    <Table size="small">
                        <TableHead>
                            <TableRow>
                                <TableCell>
                                    {isFetching ? <Skeleton variant="text" width={100} /> : 'SL'}
                                </TableCell>
                                <TableCell>
                                    {isFetching ? <Skeleton variant="text" width={100} /> : 'Container Number'}
                                </TableCell>
                                <TableCell>
                                    {isFetching ? <Skeleton variant="text" width={100} /> : 'Category'}
                                </TableCell>
                                <TableCell>
                                    {isFetching ? <Skeleton variant="text" width={100} /> : 'Action'}
                                </TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {isFetching ? [...Array(rowsPerPage < 0 ? totalRecords : rowsPerPage)].map((_, i) => (
                                <TableRow key={i}>
                                    <TableCell>
                                    ...
                                    <TableRow key={row.id}>
                                        <TableCell component="th">
                                            {((page+1) - 1) * rowsPerPage + (++index)}
                                        </TableCell>
                                        <TableCell>
                                            {row.number}
                                        </TableCell>
                                        <TableCell>
                                            {`${row.category.size_in_feet}' ${row.category.name}`}
                                        </TableCell>
                                        <TableCell>
                                            {/* <div className='flex gap-1'> */}
                                                <Tooltip title="Edit" placement="top" disableInteractive>
                                                    <Link to={`/containers/${row.id}/edit`} className='inline-block w-fit'>
                                                ...
                                                {/* <Tooltip title="update tracking" placement="top" disableInteractive>
                                                    <Link to={`/containers/${row.id}/edit`} className='inline-block w-fit'>
                                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path opacity="0.5" d="M9.5 17.5C9.5 16.5654 9.5 16.0981 9.70096 15.75C9.83261 15.522 10.022 15.3326 10.25 15.201C10.5981 15 11.0654 15 12 15C12.9346 15 13.4019 15 13.75 15.201C13.978 15.3326 14.1674 15.522 14.299 15.75C14.5 16.0981 14.5 16.5654 14.5 17.5C14.5 18.4346 14.5 18.9019 14.299 19.25C14.1674 19.478 13.978 19.6674 13.75 19.799C13.4019 20 12.9346 20 12 20C11.0654 20 10.5981 20 10.25 19.799C10.022 19.6674 9.83261 19.478 9.70096 19.25C9.5 18.9019 9.5 18.4346 9.5 17.5Z" fill="#1C274C"/>
                                                            <path opacity="0.7" d="M17 17.5C17 16.5654 17 16.0981 17.201 15.75C17.3326 15.522 17.522 15.3326 17.75 15.201C18.0981 15 18.5654 15 19.5 15C20.4346 15 20.9019 15 21.25 15.201C21.478 15.3326 21.6674 15.522 21.799 15.75C22 16.0981 22 16.5654 22 17.5C22 18.4346 22 18.9019 21.799 19.25C21.6674 19.478 21.478 19.6674 21.25 19.799C20.9019 20 20.4346 20 19.5 20C18.5654 20 18.0981 20 17.75 19.799C17.522 19.6674 17.3326 19.478 17.201 19.25C17 18.9019 17 18.4346 17 17.5Z" fill="#1C274C"/>
                        ...
                        <TableFooter>
                            <TableRow>
                                <TablePaginationWrapper pageState={{page, setPage}} rowsPerPageState={{rowsPerPage, setRowsPerPage}} totalRecords={totalRecords}/>
                            </TableRow>
                        </TableFooter>
                    </Table>
                </TableContainer>
